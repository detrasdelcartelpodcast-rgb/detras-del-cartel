...
- **Barra flotante de dispositivos** (`src/dev/DevSimulator.jsx`, solo localhost): celular / tablet / PC + rotar, arrastrable, como la de la webpage. Sin librerías nuevas. Verificado que el build de producción NO la incluye. Las BIOS de cada conductor siguen siendo texto inventado por la otra herramienta: faltan las reales de Vic y Daniel. Pendiente: botón QR para abrir en el celular (requiere escuchar en la red local; decisión de Vic).
